...
int ll_clear(LinkedList* this)
{
	...
	if(this != NULL)
	{
		while(!ll_isEmpty(this))// tambien podria ser: this->pFirstNode != NULL ||  ll_len(this) > 0
		{
...
int ll_containsAll(LinkedList* this,LinkedList* this2)
{
	int returnAux = -1;
	    ...
	    {
	    	tam = ll_len(this2);
	    	returnAux = 1; // si retorna 1, no encontro ningun elemento que no estuviera en la lista
	    	for(int i=0; i< tam;i++)
	    	{
	    		pElement = ll_get(this2, i);
	    		if(pElement != NULL && ll_contains(this, pElement) == 0)//si la lista no contiene el elemento
	    		{
	    			returnAux = 0;
	    			break;
	    		}
	...
	if(this != NULL && pFunc != NULL && order >= 0 && order <= 1)
	{
		...
		for(int i = 0; i < tam - 1; i++)
		{
			for(int j = i + 1; j < tam; j++)
			{
				...
				if((pFunc(auxElementoI, auxElementoJ) > 0 && order) || (pFunc(auxElementoI, auxElementoJ) < 0 && !order))
				{
